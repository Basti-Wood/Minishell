...
int								handle_quoted_string(const char *s, int i,
...
int								handle_operator(const char *s, int i,
									char **result, int *res_len);
int								handle_regular_char(const char *s, int i,
									char **result, int *res_len);
void							finalize_token_extraction(char *result,
									int res_len);
...
t_redir							*create_redir(char *filename,
									t_redir_type type);
...
char							*expand_dollar(char **src, char *dst,
									t_shell *shell);
char							*remove_quote_markers(const char *str);
char							*get_variable_value(t_shell *shell,
									char *var_name);
...
void							handle_format_specifier(va_list args,
									char specifier, int fd, int *len);
int								ft_printf(const char *format, ...);
int								handle_heredoc_redir(t_token **tokens,
									t_cmd *cmd, t_shell *shell);
void							add_to_ordered_redirs(t_cmd *cmd,
									t_redir *redir);
...
int								setup_child_pipes(int i, int cmd_count,
									int **pipes);
void							execute_child_external(t_cmd *current,
									t_shell *shell);
...
void							cleanup_process_resources(char *executable,
									char **env_array);
void							handle_exec_failure(char *executable,
									char **env_array);
